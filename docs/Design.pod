...
=head3 Net::BitTorrent::DHT->dump_buckets( )

This is a quick utility method which returns or prints (depending on context)
a list of the routing table's bucket structure.
...
$node->dump_buckets; # prints to STDOUT with say
my @dump = $node->dump_buckets; # returns list of lines
