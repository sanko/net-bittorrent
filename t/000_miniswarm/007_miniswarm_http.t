...
bind($httpd,
    ...
    || skip_all(sprintf q[Failed to bind tracker to port: [%d] %s], $^E, $^E);
...
diag(sprintf q[HTTP Mini-Tracker running on 127.0.0.1:%d], $_tracker_port);
...
#}
    ...
    sub check_tracker {
        ...
        if (my $paddr = accept(my ($client), $httpd)) {
            grep { $_->do_one_loop(0.1); } values %client;   # clear the pipes
            my $gotten = q[];
