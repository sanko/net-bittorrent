...
while (my $paddr = accept(my ($client), $httpd)) {
    ...
    while (sysread($client, my ($data), 1024)) {
        ...
        if ($data =~ m[^GET\s+(/(announce|scrape)\?([^\s]*))]) {
            my $type = $2;
            my %hash = split m[[=&]], $3;
            $hash{q[info_hash]} =~ s|\%([a-f0-9]{2})|pack(q[C], hex($1))|ieg;
            ...
            if ($type eq q[announce]) {
                ...
                $tracker_data{$hash{info_hash}}
                    = compact(((join q[.], @address) . q[:] . $hash{port}),
                              uncompact($tracker_data{$hash{info_hash}}));
            }
            else {    # TODO: scrape
            }
            ...
            syswrite($client,
                     sprintf <<END, scalar gmtime, bencode(\%reply)); }
