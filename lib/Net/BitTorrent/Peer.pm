        ...
        sub process_one {
            ...
            if ($read) {
                ...
                if ($actual_read) {
                    ...
                    $client{$self}->do_callback( q[peer_incoming_data], $self,
                        $actual_read );
...
qq[Improper handshake; Bad protocol name ($protocol_name)]
                ...
                $client{$self}
                  ->do_callback( q[peer_incoming_handshake], $self );
                ...
                if ( $type eq q[] ) {
                    ...
                    $client{$self}
                      ->do_callback( q[peer_incoming_choke], $self );
                }
                elsif ( $type == 1 ) {
                    if ( $packet_len != 1 ) {
                        $self->disconnect(
                    ...
                    $is_choking{$self} = 0;
                    $next_pulse{$self} =
                      Net::BitTorrent::Util::min( $next_pulse{$self},
                        time + 2 );
                    $client{$self}
                    ...
                    $client{$self}
                      ->do_callback( q[peer_incoming_request], $self,
                        $request );
                }
                elsif ( $type == 7 ) {
                    if ( $packet_len < 9 ) {
      ...
      # TODO: As we add support for more ext, this will be updated. For now, ...
        ...
        sub build_packet {
            ...
            if ( not defined $packet->{q[type]} ) {    # handshake
                ...
                $client{$self}
                  ->do_callback( q[peer_outgoing_handshake], $self );
            }
            elsif ( $packet->{q[type]} == -1 ) {       # keepalive; no payload
                $packet_data = ( pack( q[N], 0 ) );
                $client{$self}
                  ->do_callback( q[peer_outgoing_keepalive], $self );
            }
            elsif (
...
q[Unknown packet! Please, include this in your bug report: %s],
        ...
        sub generate_fast_set { # http://www.bittorrent.org/fast_extensions.html
            ...
            my $x = sprintf(
                ...
                (
                    0xFFFFFF00 &
                      ( hex unpack q[H*], pack q[C*], $ip =~ m[(\d+)]g )
                )
            );
