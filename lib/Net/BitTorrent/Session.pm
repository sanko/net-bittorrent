{
    ...
    {
        my (
            %next_pulse,  %path,     %base_dir,   %private,
...
=cut
        ...
        sub new {
            ...
            if (defined $args->{q[client]}) {
                if (not defined $args->{q[path]}) {
                    ...
                    my ($_content) = Net::BitTorrent::Util::bdecode($_data);

                    my $infohash =
                      Digest::SHA::sha1_hex(
                        Net::BitTorrent::Util::bencode($_content->{q[info]}));
                    if ($infohash !~ m[^([0-9a-f]{40})$]) {
                        croak(q[Improper info_hash]);
                        return;
                    }
                    elsif (
                        length(unpack(q[H*], $_content->{q[info]}{q[pieces]})) <
                        40)
                    {
                    ...
                    $piece_size{$self} = $_content->{q[info]}{q[piece length]};
        ...
        sub base_dir    { return File::Spec->rel2abs($base_dir{+shift}); }
        ...
        sub append_nodes {
            my ($self, $new_nodes) = @_;
            my @old_nodes = Net::BitTorrent::Util::uncompact($nodes{$self});
            my @new_nodes = Net::BitTorrent::Util::uncompact($new_nodes);
            my @all_nodes = (@old_nodes, @new_nodes);
            return $nodes{$self} = Net::BitTorrent::Util::compact(\@all_nodes);
        }
        sub nodes { return Net::BitTorrent::Util::uncompact($nodes{+shift}) }
        sub compact_nodes { return $nodes{+shift}; }    # quick-resume
        ...
        sub pulse {
            ...
            my @peers = $self->peers;
            for my $peer (@peers) { $peer->pulse if $peer->next_pulse < time; }
...
# TODO: review the following block =============================================
            ...
            for my $piece (
                grep {
                    $_->working
                      and ((time - $_->previous_incoming_block) > 1200)
                } @{$pieces{$self}}
              )
            {
                $piece->working(0);
                ...
                warn(
                    sprintf(
                        ...
                        $piece->index, $piece->working, $piece->priority
                    )
...
# TODO: review the above block =================================================
            ...
            warn sprintf q[-----> Current peers: half-open:%d | mine:%d],
              scalar(grep { not $_->peer_id } @peers), scalar(@peers)
              if $Net::BitTorrent::DEBUG;
            ...
            if (
                    $self->nodes
                and (scalar(@peers) < $self->client->maximum_peers_per_session)
                and (
                    scalar(
                        grep {
              ...
              )
            {
                  ...
                  )
                {
                    my $new_peer = Net::BitTorrent::Peer->new(
                        {address => shift(@nodes), session => $self});
                    $self->client->add_connection($new_peer) if $new_peer;
                }
            }
        ...
        sub pick_piece {
            ...
            warn sprintf qq[max: %d, w: (%d) %s\n],
              $max_working,
              scalar @weights, join(q[, ], map { $_->index } @weights)
              if $Net::BitTorrent::DEBUG;

            if   (scalar @weights < 3) { $endgame{$self} = 1 }
            else                       { $endgame{$self} = 0 }

            # [id://230661]
            my $total =
              Net::BitTorrent::Util::sum map { $_->priority } @weights;
            my $rand_val = $total * rand;
            my $i        = -1;
            while ($rand_val > 0) { $rand_val -= $weights[++$i]->priority }
            $piece = $weights[$i];
        ...
        sub complete {
            for my $piece (@{$pieces{+shift}}) {
                if (not $piece->check and $piece->priority) { return 0 }
            }
            return 1;
        }
...
END
                ...
                }
                {
                    my @adv_values_piece_info = (
                        ...
                        join(q[, ],
                            map { $_->index }
                              grep { $_->blocks and not $_->working }
                              @{$pieces{$self}})
                    );
                    s/(^[-+]?\d+?(?=(?>(?:\d{3})+)(?!\d))|\G\d{3}(?=\d))/$1,/g
