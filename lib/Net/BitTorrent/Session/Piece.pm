{
    ...
    {    # constructor
        my (%hash, %index, %session, %blocks, %check, %touch, %priority,
        ...
        sub previous_incoming_block {
            ...
            return (
                defined $value
                ? $previous_incoming_block{$self} = $value
            ...
            return (
                ...
                ? do {
                    ...
                    if ($value and not $blocks{$self}) {
                        ...
                        my $_block_length =
                          Net::BitTorrent::Util::min($self->session->block_size,
                            $_length);
        ...
        sub read {
            ...
            croak(q[Bad offset!]) if defined $offset and $offset !~ m[^\d+$];

            my $_RETURN = q[];
            my $_LENGTH = defined $length ? $length : $self->size;

            my $pos = int(
                (($self->index * $self->session->piece_size)) + ($offset || 0));

            if (($pos + $_LENGTH) > $self->session->total_size) { return; }
            my $f = 0;
            ...
            while ($_LENGTH > 0) {
                ...
                $_RETURN .= $self->session->files->[$f]->read($this_read)
                  or return;
        ...
        sub verify {
            ...
            if (Digest::SHA::sha1($self->read) eq $self->hash) {
                ...
                $session{$self}->client->do_callback(q[piece_hash_pass], $self)
                  if not
                      $old_value;  # no point telling us the piece is good twice
                return 1;
            }

            # failed
            # TODO: penalize all peers related to piece
            $session{$self}->client->do_callback(q[piece_hash_fail], $self);
            return 0;
        }
