...
The bit selected for the extension protocol is bit C<20> from the right
(counting starts at C<0>). So (C<reserved_byte[5] & 0x10>) is the expression
to use for checking if the client supports extended messaging.

Once support for the protocol is established, the client is supposed to
support C<1> new message:
...
This message is sent as any other bittorrent message, with a C<4> byte length
prefix and a single byte identifying the message (the single byte being C<20>
...
and only one ID is specified, C<0>. If the ID is C<0>, the message is a
handshake message which is described below. The layout of a general extended
message follows (including the message headers used by the bittorrent
protocol):
...
=head1 handshake message

The payload of the handshake message is a bencoded dictionary. All items in
the dictionary are optional. Any unknown names should be ignored by the
client. All parts of the dictionary are case sensitive. This is the defined
...
The handshake dictionary could also include extended handshake information,
such as support for encrypted headers or anything imaginable.
...
This message should be sent immediately after the standard bittorrent
handshake to any peer that supports this extension protocol. It is valid to
send the handshake message more than once during the lifetime of a connection,
the sending client should not be disconnected. An implementation may choose to
ignore the subsequent handshake messages (or parts of them).

Subsequent handshake messages can be used to enable/disable extensions without
restarting the connection. If a peer supports changing extensions at run time,
it should note that the C<m> dictionary is additive. It's enough that it
contains the actual B<CHANGES> to the extension list. To disable the support
for C<LT_metadata> at run-time, without affecting any other extensions, this
message should be sent: C<d11:LT_metadatai0ee>.

As specified above, the value C<0> is used to turn off an extension.
...
peer-exchange or metadata exchange. This protocol is merely a transport for
the actual extensions to the bittorrent protocol and the extensions named in
the example above (such as C<p>) are just examples of possible extensions.
...
implementing the extension message. e.g. C<LT_metadata> is implemented by
libtorrent, and hence it has the C<LT> prefix.
...
they can be used in C<switch> statements. It's easy for the other end to store
an array with the ID's we expect for each message and use that for lookups
each time it sends an extension message.

The reason for having a dictionary instead of having an array (using
implicitly assigned index numbers to the extensions) is that if a client want
to disable some extensions, the ID numbers would change, and it wouldn't be
able to use constants (and hence, not use them in a C<switch>). If the
...
The reasoning behind having a single byte as extended message identifier is to
follow the the bittorrent spec. with its single byte message identifiers. It
is also considered to be enough. It won't limit the total number of
...
identifiers is C<1>) The mainline DHT uses single byte identifiers. C<2>)
Saves bandwidth. The only advantage of longer messages is that it makes the
