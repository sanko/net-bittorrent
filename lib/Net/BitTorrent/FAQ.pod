...
will be judged on how fast they transfer pieces to us, how often they
are involved with 'bad' pieces, how many times they have choked us
while we were waiting for a piece, etc.  Making attempts to stay
...
Private versions of N::B had resume built in but it was removed for
...
some sort of anti-tampering/verification scheme to be sure you're
loading the right resume info.  Then, when you load the torrent, set
the C<skip_hashcheck> parameter to true and reload the torrent with
your stored data. ...well, as long as the modified times match the
current timestamps and the data verifies.
...
why.  If it turns out you alone were banned (because you're a leech
or you modified the source, etc.), I cannot (and would not) help you.
...
Artic (L<http://dev.int64.org/arctic.html>) is a minimal client
